...
bool EachVsEachTournamentUI::onEvent(const Event& event) const
{
	...
	case Event::CantDetermineTheWinner:
		...
		if (tournamentPtr->getPlayers().size() == 2)
		{
			...
			std::cout << "So it's the Duel then with rules of the tournament between " << tournamentPtr->getPairOfCurrentRoundPlayers().first->getName()
				<< " and " << tournamentPtr->getPairOfCurrentRoundPlayers().second->getName() << "\n";
			Sleep(5000);
